...
//       canvas/             (← a symlink to this repo = the <id> dir)
//
// `../../types` (the HOST contract) resolves up to .verify/types.ts — the
// production position. The ui imports `@frontierengineer/ui` (the host's shared
// primitives); the host bundler aliases that to its frontend tree, so for the
// local typecheck we point it at the vendored hostUi.d.ts shim via `paths`. We
// run tsc from the mirror (ui), then esbuild every entry the way the bundler
// does. The mirror is throwaway (.gitignored); the committed source stays a
// clean, flat, production-faithful extension.
...
// ── Run the checks ─────────────────────────────────────────────────────────
console.log('Verifying canvas (production-nested mirror in .verify/):\n');
if (presentHostCaps.length) {
  ...
  fs.writeFileSync(hostTsconfig, JSON.stringify({
    extends: `./${id}/server/tsconfig.json`,
